...
wdir = '[DIRECTORY PATH]/NFDRS_services'

# ArcGIS Online Portal URL
agol_portalurl = 'https://www.arcgis.com'

# Service item ID
erc_itemid = '[SERVICE ITEM ID]'

# ArcGIS Online Username
agol_username = '[AGOL USERNAME]'

# ArcGIS Online Password
agol_password = '[AGOL PASSWORD]'

# Toggle for which day to use for WIMS data pull. Specify either 'Current Date' or 'Previous Date'
toggle_run_date = 'Current Date'
#toggle_run_date = '2024-06-23 14:00:00'
...
allstations = pandas.read_csv('[DIRECTORY PATH]/AllStation.csv',
                              converters={'StationID': str})
percentiles = pandas.read_csv('[DIRECTORY PATH]/Percentiles.csv',
                              converters={'StationID': str})
    ...
    try:
        ...
        for k in range(0, len(raws_update_columns)):
            try:
                ...
                #if( (bad_merge == True) or (latest_obs_date_str != date_today_str_yyyymmdd)): #######Testing change 
                if( latest_obs_date_str != datetime_today.strftime('%Y%m%d') ):
...
print_both('.PSA\r')
#psa_update_fset = arcgis.features.FeatureSet.from_dataframe(psa_update_sdf)
#psa_layer.edit_features(updates = psa_update_fset)
# Switched to updates by GACC to mitigate data size issues
...
raws2psa_df.to_csv(wdir + '/raws2psa_data_TESTING.csv')
raws_update = raws_update_sdf.drop('SHAPE',axis=1)
raws_update.to_csv(wdir + '/raws_data_TESTING.csv')
psa_update = psa_update_sdf.drop('SHAPE',axis=1)
psa_update.to_csv(wdir + '/psa_data_TESTING.csv')
